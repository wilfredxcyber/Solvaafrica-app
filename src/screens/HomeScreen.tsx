...
export default function HomeScreen() {
    ...
    return (
        <View style={globalStyles.screen}>
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: hscale(20) }}>
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    <AvatarView />
                    <View style={{ marginLeft: 8 }}>
                        <Text style={styles.greetUserText}>Hello, {userData.fullName.split(' ')[0]}</Text>
                        <Text style={[globalStyles.bodyText, { fontSize: mscale(14) }]}>Here's what is happening today.</Text>
                    </View>
                </View>
            ...
            <Carousel
                ...
                mergeStyles={true}
            >
                {slidesImages.length ? slidesImages.map((currentSlideImage, index) => (
                    <View style={{ width: width - (screenHorizontalPadding * 2), height: '100%' }} key={index + currentSlideImage}>
                        <Image source={{ uri: currentSlideImage.url }} style={{ width: '100%', height: '100%' }} />
                    </View>
                )) : new Array(3).map((_, index) => (
                    <View style={{ width: width - (screenHorizontalPadding * 2), height: '100%' }} key={index}>
            ...
            <View style={styles.menuView}>
                {MENU_ITEMS.map((currentItem, index) => (
                    <Pressable key={currentItem.item + index} style={styles.menuItemView} onPress={() => handleMenuItemPressed(currentItem.item)}>
                        <View style={styles.menuItemIconView}>
                            {currentItem.icon()}
                        </View>
                        <Text style={{ textAlign: 'justify', marginTop: 8, fontFamily: 'Inter-Medium', color: colors.bodyText, fontSize: mscale(12) }}>{currentItem.item}</Text>
                    </Pressable>
                ))}
            </View>
        </View>
