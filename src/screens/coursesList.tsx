...
export default function CoursesList({ route }: ScreenProps) {
    ...
    useEffect(() => {
        ...
        const fetchCourses = async () => {
            ...
            try {
                const res = await AUTH_API_CLIENT.get('/questions', { params: { university, department, faculty } })
    ...
    return (
        <View style={{ flex: 1, backgroundColor: '#ffffff' }}>
            {!coursesList.length ? <Text style={styles.noFilesFoundText}>No files found.</Text> : <FlashList
                data={coursesList}
...
const CoursesListItem = ({ courseTitle, courseCode, university, previewUrl, courseId }: CourseListItemProps) => {
    const navigation = useNavigation();

    const handleCourseListItemPressed = () => {
        navigation.navigate("App", { screen: 'CourseMaterials', params: { courseId, headerTitle: `${courseTitle} Materials` } })
    }
    return (
        <Pressable
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', flex: 1, marginLeft: wscale(8), alignItems: 'center' }}>
                <View>
                    {/* course title */}
                    <Text style={{ fontFamily: 'Inter-Regular', color: colors.bodyText, fontSize: mscale(16) }}>{courseTitle}</Text>
                    {/* course code */}
                    <Text style={{ fontFamily: 'Inter-Bold', color: colors.bodyText, fontSize: mscale(16) }}>{courseCode}</Text>
                    {/* university */}
                    <Text style={{ fontFamily: 'Inter-Bold', color: colors.primary, fontSize: mscale(14) }}>{university}</Text>
                </View>
...
const styles = StyleSheet.create({
    coursesListImage: { height: hscale(60), width: wscale(60), borderRadius: mscale(8) },
    noFilesFoundText: { fontFamily: 'Inter-Bold', fontSize: mscale(24), color: colors.black, textAlign: 'center' }
})