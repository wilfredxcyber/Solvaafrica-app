...
export default function CoursesScreen() {
    ...
    const handleSearch = () => {
        navigation.navigate('App', { screen: 'CoursesList', params: { searchListParams: { university, faculty, department } } })
    }
    ...
    return (
        <ProtectPage>
            <View style={globalStyles.screen}>
                <View style={{ gap: 12 }}>
                    {/* university */}
                    <DropDownPicker data={universities} setSelectedValue={setUniversity} defaultValue={university} />
                    {/* faculties */}
                    <DropDownPicker data={facultyList} setSelectedValue={setFaculty} defaultValue={faculty} />
                    {/* departments */}
                    <DropDownPicker data={departmentList} setSelectedValue={setDepartment} defaultValue={department} />
...
const DropDownPicker = ({ data, setSelectedValue, defaultValue }: DropdownPickerProps) => {
    ...
    const handleInputBlur = () => { console.log('Blurred') }
    ...
    const renderItem = ({ item, index }: { item: string, index: number }) => {
        return <Text onPress={() => handleListItemPressed(item)} style={[styles.dropDownListItem, { borderBottomWidth: index + 1 === data.length ? 0 : StyleSheet.hairlineWidth }]}>{item}</Text>
    }
    ...
    return (
        <View ref={dropdownViewRef}>
            ...
            <Pressable onPress={handleDropdownInputPressed} style={styles.dropdownInputContainer}>
                ...
                />
                <DropdownIcon name={!dropdownIsVisibile ? "chevron-small-up" : "chevron-small-down"} size={20} color={colors.black} />
            </Pressable>

            {/* items list */}
            {dropdownIsVisibile && (
                <Modal transparent={true}>
                    <Pressable onPress={() => setDropdownIsVisibile(false)} style={{ paddingHorizontal: screenHorizontalPadding, height: Dimensions.get('screen').height }}>
                        <View style={[styles.dropDownContainer, { width: '100%', height: hscale(300), marginTop: dropdownTopMargin }]}>
                            <FlashList
                                data={data}
